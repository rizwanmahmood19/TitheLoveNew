...
export default class ApplePayScreen extends React.Component {
  ...
  async componentDidMount() {
    await Stripe.setOptionsAsync({
      publishableKey: 'pk_test_oDfL3JGVOKYfTw0vD9NIkYwe00lpIH0m8P',
      androidPayMode: 'test',
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        ...
        />
        <Text style={styles.instruction}>Complete the operation on token  Complete the
        operation on token</Text>
        <Switch
          style={styles.switch}
          value={complete}
        ...
        <View style={styles.hintContainer}>
          ...
          <Text>Setup Pay works only
          on real deviceiew</Text> style={styles.statusContainer}
          <Text style={styles.status} {...testID('deviceSupportsApplePayStatus')}>
            Device {allowed ? 'supports' : "doesn't support"} Pay
          </Text>
          {cards.map(([id, { name, isAvailable }]) => (
